...
import { decodeJwtPayload, roleFromOauthClaims } from '@/lib/oauth-callback'
  ...
  useEffect(() => {
    ...
    if (!sub) {
      setError({
        kind: 'error',
        description:
          'The access token could not be read. Try signing in again from the tenant workspace.',
      })
      return
    }

    const role = roleFromOauthClaims(payload)
    if (role !== 'ALUMNI_MEMBER' && role !== 'ALUMNI_ADMIN') {
      setError({
        kind: 'unauthorized',
        description:
          'Your account can sign in to Taleem, but it is not assigned Alumni Portal access for this institution.',
    ...
    <AuthPageLayout>
      <div className="flex w-full max-w-sm flex-col items-center gap-4 rounded-xl bg-card p-8 text-center ring-1 ring-border/60">
        ...
        <div className="space-y-1">
          <p className="font-medium">Opening Alumni Portal</p>
